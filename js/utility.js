/**
 * TeleFlow - Utility module (Backup Data / Change Password / Settings).
 */
...
function renderBranding() {
    const settings = lfGetSettings();
    const name = settings.companyName || 'TeleFlow';
...
function exportBackup() {
    const backup = { _meta: { app: 'TeleFlow', exportedAt: new Date().toISOString() } };
    ...
    a.href = url;
    a.download = `TeleFlow-Backup-${new Date().toISOString().slice(0, 10)}.json`;
...
function handleRestoreFile(input) {
    ...
    if (!confirm('This replaces everything currently in TeleFlow with the contents of this file. This cannot be undone. Continue?')) {
        input.value = '';
        return;
    }
    ...
    reader.onload = (event) => {
        (async () => {
            try {
                ...
                showToast("That file doesn't look like a valid TeleFlow backup, or the restore didn't fully complete.", 'error', 6000);
            } finally {
                input.value = '';
            }
        })();
