...
function shareInvoiceWhatsApp(invId) {
    ...
    let text = `*${s.companyName || 'LedgerFlow'}*\n`;
    text += `*${data.title} — ${data.number}*\n`;
    text += `Date: ${data.date}\n`;
    text += `${data.partyLabel}: ${data.partyName}\n\n`;

    if (data.hasLoad) {
        text += `Load Amount: ${formatCurrency(data.loadAmount)}\n`;
        text += `Discount: ${data.loadDiscount}%\n`;
        text += `Load Qty: ${(data.loadQty || 0).toLocaleString('en-US')}\n`;
        text += `Load Total: *${formatCurrency(data.loadTotal)}*\n\n`;
    }

    if (data.items.length) {
        data.items.forEach((it, i) => {
            text += `${i + 1}. ${it.name} — ${it.qty} x ${formatCurrency(it.rate)} = ${formatCurrency(it.amount)}\n`;
        });
        text += `Items Total: *${formatCurrency(data.itemsTotal)}*\n\n`;
    }

    text += `*Grand Total: ${formatCurrency(data.grandTotal)}*\n`;
    if (data.paymentMode && data.paymentMode !== 'None' && data.paymentAmount > 0) {
        text += `Paid (${data.paymentMode}): ${formatCurrency(data.paymentAmount)}\n`;
    }
    if (data.balanceAmount > 0) {
        text += `Balance: ${formatCurrency(data.balanceAmount)}\n`;
    }
    if (data.cancelled) text += `\n⛔ CANCELLED`;
