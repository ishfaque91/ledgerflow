/**
 * TeleFlow - Authentication module (Firebase Auth version).
 *
...
async function resolveCompanyAndShowApp(user) {
    try {
        ...
        watchCollection(LF_KEYS.BTL_VERIFICATIONS, () => { if (typeof renderBtlVerification === 'function') renderBtlVerification(); });
...
// which destroys the TeleFlow mark's <svg> by overwriting it with plain
// text. Once signed out there's no company in scope, so put the generic
// TeleFlow identity back.
function resetLoginBranding() {
    $('login-company-name').textContent = 'TeleFlow';
