/**
 * TeleFlow - Shared utility helpers used across every module.
 */
...
function makeSearchableSelect(selectEl) {
    ...
    function buildOptions(filter = '') {
        ...
        [...selectEl.options].forEach(opt => {
            ...
            const selected = opt.value === selectEl.value ? ' is-selected' : '';
            html += `<div class="ss-option${selected}" data-value="${escapeHtml(opt.value)}">${escapeHtml(text)}</div>`;
        });
        optionsList.innerHTML = html || '<div class="ss-no-match">No matches</div>';
    }

    function updateDisplay() {
        const opt = selectEl.options[selectEl.selectedIndex];
        displayText.textContent = opt ? opt.textContent : 'Select…';
        displayText.classList.toggle('ss-placeholder', !selectEl.value);
    }
    ...
    selectEl._ssUpdate = () => updateDisplay();
    selectEl._ssRefresh = () => { buildOptions(); updateDisplay(); };
}

function initSearchableSelects() {
    document.querySelectorAll('select.searchable, .modal select, .report-field select').forEach(makeSearchableSelect);
}
