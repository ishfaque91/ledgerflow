...
// ==================== PETTY CASH ====================
function setPcDirection(dir) {
    $('pc-direction').value = dir;
    document.querySelectorAll('#pc-direction-group .btn-toggle').forEach(b => {
        b.classList.toggle('is-active', b.dataset.value === dir);
    });
    $('pc-cash-label').textContent = dir === 'Payment' ? 'Paid From (Account)' : 'Received Into (Account)';
}
...
function openPettyCashForm(editId = null) {
    ...
    if (editId) {
        ...
        setPcDirection('Payment');
        if (allAccounts.length) $('pc-cash-account').value = allAccounts[0].id;
        addPettyCashLine();
    }
...
async function savePettyCash() {
    ...
    const pcDirection = $('pc-direction').value || 'Payment';
    const lines = getPettyCashLines();
    const saveBtn = $('pettycash-save-btn');

    if (!date) { showToast('Please choose a date.', 'warning'); return; }
    if (!cashAccountId) { showToast('Please select the main account.', 'warning'); return; }
    if (lines.length === 0) { showToast('Add at least one line with an amount.', 'warning'); return; }

    const pcCancelled = !!$('pc-cancelled').checked;

    setBtnLoading(saveBtn, true);

    const isPayment = pcDirection === 'Payment';
    const lineSide = isPayment ? 'Dr' : 'Cr';
    const cashSide = isPayment ? 'Cr' : 'Dr';
